// ========================================================================
// PEŁNY KOD script.js (WERSJA z ZIELONĄ MURAWĄ, LINIAMI i TŁEM MENU - PONOWNIE)
// ========================================================================
(function () {
    "use strict";
    ...
    // --- INICJALIZACJA GRY ---
     function initGame() {
         ...
         canvas.width = 640; // Szerokość logiczna
         canvas.height = 400; // Wysokość logiczna
         resizeCanvas(); // Ustawia styl CSS (widoczny rozmiar)
    ...
    // --- FUNKCJE RYSOWANIA ---
     function drawField() {
         ...
         // ===================================================
         // === TUTAJ USTAWIANY JEST KOLOR TŁA MURAWY ===
         ctx.fillStyle = "#228B22"; // ForestGreen (ZIELONY)
         // ===================================================

         // Rysowanie prostokąta murawy
         ctx.fillRect(0, 0, canvas.width, canvas.height);

         // === Rysowanie linii boiska ===
         ctx.strokeStyle = "rgba(255, 255, 255, 0.5)"; // Półprzezroczyste białe linie
         ctx.lineWidth = 2;
         ...
         // Wymiary pól
         const penaltyAreaWidth = 105; const penaltyAreaHeight = 260;
         const goalAreaWidth = 35; const goalAreaHeight = 110;
         const penaltySpotDist = 70; const penaltyArcRadius = 55;
         ...
         // Punkty karne
         ctx.beginPath(); ctx.arc(penaltySpotDist, canvas.height / 2, 3, 0, Math.PI * 2); ctx.fill();
         ctx.beginPath(); ctx.arc(canvas.width - penaltySpotDist, canvas.height / 2, 3, 0, Math.PI * 2); ctx.fill();
         // Łuki karne
         ctx.beginPath(); ctx.arc(penaltySpotDist, canvas.height / 2, penaltyArcRadius, -Math.PI * 0.34, Math.PI * 0.34); ctx.stroke();
         ctx.beginPath(); ctx.arc(canvas.width - penaltySpotDist, canvas.height / 2, penaltyArcRadius, Math.PI - Math.PI * 0.34, Math.PI + Math.PI * 0.34); ctx.stroke();

         // Bramki (słupki)
        const goalPostWidth = 5; const goalVisualHeight = 80;
    ...
    function gameLoop() {
         ...
         // 1. Wyczyść cały canvas
         ctx.clearRect(0, 0, canvas.width, canvas.height);

         // =======================================================
         // === 2. Narysuj tło boiska (zielona murawa i linie) ===
         drawField();
         // =======================================================

         // 3. Zaktualizuj fizykę i logikę
         updatePositions();
         checkCollisions();
         aiMove();

         // 4. Narysuj obiekty na wierzchu murawy
         drawGameObjects();

         // 5. Poproś o następną klatkę
    ...
    // --- GŁÓWNA INICJALIZACJA (DOMContentLoaded) ---
     document.addEventListener("DOMContentLoaded", () => {
         ...
         if (backToStartBtn) backToStartBtn.addEventListener("click", () => {
             gameAnimating = false; stopTimer(); closeModal(gameScreen); openModal(startScreen);
             if(startScreen) startScreen.classList.add('start-screen-background'); // Przywróć tło menu
         ...
         console.log("MiniSoccer - Inicjalizacja zakończona (ponowna próba z zieloną murawą).");
         resizeCanvas();
     });
