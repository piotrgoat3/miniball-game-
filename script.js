// ========================================================================
// PEŁNY KOD script.js (WERSJA z przezroczystym canvasem - jak v15/v17)
// ========================================================================
(function () {
    "use strict";
    ...
    // --- FUNKCJE RYSOWANIA ---
     function drawField() {
         if (!ctx) return;
         // PUSTE - Nie rysujemy boiska na canvasie
     }
    ...
    // --- GŁÓWNA INICJALIZACJA (DOMContentLoaded) ---
     document.addEventListener("DOMContentLoaded", () => {
         ...
         if (startMatchBtn) startMatchBtn.addEventListener("click", () => {
             console.log("Szybki Mecz Clicked"); // Dodajmy log do sprawdzenia
             closeModal(startScreen);
             openModal(teamSelectScreen);
             try { // Dodajmy try...catch wokół populate, na wszelki wypadek
                populateTeamSelections();
             } catch (error) {
                 console.error("Error in populateTeamSelections:", error);
             }
         });
